...
void new_tree(struct edge tree[]);
void ins_Q(int i, int j, int wt);
...
void make_a_graph()
{
      ...
      cout<<"TO EXIT, enter -1 -1\n\n";
      for(count = 0; count < maximum_edges; count++)
      {
            printf("\nEnter Edge [%d] Co-ordinates \n", count + 1);
            ...
            {
                  ins_Q(origin_vertex, destination_vertex, weight);
            }
      }
}
